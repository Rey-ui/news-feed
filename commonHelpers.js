            ...
            <use href="${c}#icon-inbox-paper"></use>
                  ...
                  <div class="hero__item-content">
                    ...
                    <div class="hero__item-text">
                      ...
                      <p class="hero__item-description">${l||"there are no details on this post..."}</p>
                    </div>
            ...
            <use href="${c}#icon-inbox-paper"></use>
                  ...
                  <div class="news-search__item-content">
                    ...
                    <div class="news-search__item-text">
                      ...
                      <p class="news-search__item-description">${l||"there are no details on this post..."}</p>
                    </div>
              ...
              </li>`).join("");e.insertAdjacentHTML("beforeend",o)}const d={page:null,maxPage:0,country:"us",category:"top",q:null,size:4};async function f(e,s,o){d.q=e,d.category=s,d.country=o,b(r.newsSearchLoader);try{const{results:n,nextPage:t}=await w(d);if(i.latest.length=0,i.latest.push(...n),!n.length){d.country="us",p.error({title:"Error",message:"❌Sorry, nothing was found for your request!"});return}d.page=t,r.newsSearchList.innerHTML="",P(r.newsSearchList,i.latest),t?L(r.loadMoreBtnNewsSearch,_):m(r.loadMoreBtnNewsSearch,_)}catch(n){console.log(n),p.error({title:"Error",message:"❌Sorry, nothing was found for your request!"})}finally{S(r.newsSearchLoader)}}async function _(){$(r.loadMoreBtnNewsSearch,r.newsSearchPreLoader);try{const{results:e,nextPage:s}=await w(d);i.latest.push(...e),P(r.newsSearchList,e),d.page=s,s||(p.info({title:"Message",message:"You've reached the end of results"}),m(r.loadMoreBtnNewsSearch,_))}catch{}finally{M(r.loadMoreBtnNewsSearch,r.newsSearchPreLoader)}}async function D(e){e.preventDefault();const s=e.currentTarget,o=s.elements.searchInput.value||null,n=s.elements.formCountrySelect.value,t=s.elements.categoryRadio.value;f(o,t,n)}r.clearBtn.addEventListener("click",()=>{const e=r.newsSearchForm,s=e.elements.searchInput.value||null,o=e.elements.formCountrySelect.value,n=e.elements.categoryRadio.value;!s&&o=="us"&&n==="top"||(e.reset(),f(null,"top","us"))});function I(e){e.target!==e.currentTarget&&r.headerBurgerEl.classList.remove("active"),r.headerBurgerEl.classList.add("active")}function O(e){r.headerBurgerEl.classList.remove("active")}N();f();r.headerBurgerBtn.addEventListener("click",I);r.headerBurgerBtnClose.addEventListener("click",O);r.topHeadlinesHero.addEventListener("click",e=>{B(e,i.hero,".hero__item")});r.newsSearchList.addEventListener("click",e=>{B(e,i.latest,".news-search__item")});r.newsSearchForm.addEventListener("submit",D);document.addEventListener("error",function(e){if(e.target.tagName==="IMG"){e.target.style.display="none";const s=e.target.nextElementSibling;s&&(s.style.display="flex")}},!0);
